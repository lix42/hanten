...
Real-scan verification of `film-base/estimation` (see
`docs/progress/film-base.md`) showed the current heuristic samples the outer 4%
margin as one blob — holder + rebate + picture — which is high-spread, so it
always bails. The rebate (the actual film
