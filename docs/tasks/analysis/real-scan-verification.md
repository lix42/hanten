...
verification matrix (results recorded in `docs/progress/analysis.md`) plus a
follow-up task for every defect found. This task intentionally does not wait for
future display/HDR outputs; the user prepares the assets.
...
Every row gets pass/fail + the observed numbers in `docs/progress/analysis.md`.
A failure becomes a new tracked task (via the task-update flow) rather than an
ad-hoc fix inside this one — this task's output is *knowledge*, not patches.
...
every asset class with results recorded in `docs/progress/analysis.md`, and a
filed follow-up task (or explicit "none") for defects. No code changes expected;
if any prove necessary they go through their own tasks.
...
[`dense-base-dmax-plausibility`](../film-base/dense-base-dmax-plausibility.md) (Phoenix);
...
- [Display-range white anchor (Dmax)](../algo/dmax-white-anchor.md)
