...
Let a review set place an **outside producer's image** beside Hanten's renders of the same
frame: a Negative Lab Pro export, a SmartConvert TIFF, or the user's own hand-tweaked
target. Today every cell of a `nctool review generate` matrix is a `hanten convert`, so
"how does Hanten compare with what I actually wanted" cannot be put in front of the eye.
...
- References differ from renders in encoding and framing: NLP exports are 32-bit float
  ...
  comparison is confounded by Hanten decoding as HDR beside NLP's SDR. Cropped references
...
1. Matrix syntax: a cell kind (`{"reference": "<path>", "space": "adobe-rgb"}`) beside
   the `hanten convert` cells, or a separate list resolved through the manifest by
   `source_frame`?
...
- A matrix with one reference per frame renders a set where the reference toggles in
  place with Hanten's renders, at the same grid position, with its declared space recorded.
