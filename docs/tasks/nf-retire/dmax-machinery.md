...
- **The effective area stays.** It is resolved, warned on and reported on every
  ...
  leaves `measure.inset` without its one consumer on the current chain. The new
  chain has one since `nf-scene-correction/roll-white-balance`: `hanten measure-roll`
  samples each frame over the effective area, so the knob stays live there.
