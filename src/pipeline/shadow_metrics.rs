//! Test-only diagnostic harness from `algo/reference-anchored-sigmoid`: the shadow-patch
//! proposal and the reference-frame characterisation, both read in density.
//!
//! The candidate-anchor, tone-map, linear-render, reconstruction-shape, HDR-gain and
//! curve-colour probes that also lived here rendered through the sigmoid and were deleted
//! with it (`nf-retire/sigmoid-and-simple`) rather than re-pointed at another curve — a
